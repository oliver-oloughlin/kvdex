...
Deno.bench(
  "indexable_collection - updateOneBy (replace) [1_000]",
  async (b) => {
    await useDb(async (db) => {
      ...
      b.start();
      await db.i_users.updateOneBy("age", 25, mockUser1, {
        strategy: "replace",
      });
      b.end();
    });
...
Deno.bench(
  "indexable_collection - updateOneBy (shallow merge) [1_000]",
  async (b) => {
    await useDb(async (db) => {
      const targetUsers = generateUsers(1_000, 25);
      const otherUsers = generateUsers(1_000, 50);
      ...
      b.start();
      await db.i_users.updateOneBy("age", 25, updateData, {
...
Deno.bench(
  "indexable_collection - updateOneBy (deep merge) [1_000]",
  async (b) => {
    await useDb(async (db) => {
      const targetUsers = generateUsers(1_000, 25);
      const otherUsers = generateUsers(1_000, 50);
