  ...
  await t.step(
    ...
    async () => {
      await useKv(async (kv) => {
        ...
        const { result: docs1 } = await db.numbers.getMany()
        const { result: docs2 } = await db.arrays.getMany()
        const { result: docs3 } = await db.dates.getMany()

        const ids1 = docs1.map((doc) => doc.id)
        const ids2 = docs2.map((doc) => doc.id)
        const ids3 = docs3.map((doc) => doc.id)

        const versionstamps1 = docs1.map((doc) => doc.versionstamp)
        const versionstamps2 = docs2.map((doc) => doc.versionstamp)
        const versionstamps3 = docs3.map((doc) => doc.versionstamp)
