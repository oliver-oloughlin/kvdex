  ...
  async (t) => {
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const { result } = await db.is_users.updateManyBySecondaryOrder(
            "age",
            updateData,
            {
              limit: 2,
    ...
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const { result: crs } = await db.is_users.updateManyBySecondaryOrder(
            "age",
            updateData,
            {
              strategy: "replace",
    ...
    await t.step("Should successfully parse and update", async () => {
      await useDb(async (db) => {
        ...
        await db.zis_users.updateManyBySecondaryOrder("age", mockUser1)
    ...
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const docs = await db.is_users.getManyBySecondaryOrder("age");
