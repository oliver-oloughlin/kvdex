...
Deno.test("serialized_indexable_collection - properties", async (t) => {
  ...
  await t.step("Keys should have the correct prefixes", async () => {
    await useDb((db) => {
      ...
      const prefix = extendKey([DEFAULT_BASE_KEY_PREFIX], "is_users");
  ...
  await t.step("Should generate ids with custom id generator", async () => {
    await useKv((kv) => {
      ...
      const id1 = db.users1["idGenerator"](user);
      const id2 = db.users2["idGenerator"](user);
  ...
  await t.step("Should allow optional indices", async () => {
    await useKv(async (kv) => {
      ...
      const byOptPrimary2 = await db.is.findBy(
        "optPrimary",
        "optPrimary2",
      );
      const byOptSecondary2 = await db.is.getManyBy(
        "optSecondary",
        20,
      );
      ...
      const byOptPrimary4 = await db.is.findBy(
        "optPrimary",
        "optPrimary4",
      );
      const byOptSecondary4 = await db.is.getManyBy(
        "optSecondary",
        20,
      );
