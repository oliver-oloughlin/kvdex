...
Deno.test("serialized_indexable_collection - getManyBy", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        const t1 = await validate(TransformUserModel, mockUser1);
        const t2 = await validate(TransformUserModel, mockUser2);

        const cr = await db.ai_users.addMany([mockUser1, mockUser2]);
        assert(cr.ok);

        const { result } = await db.ai_users.getManyBy(
          "decadeAge",
          t1.decadeAge,
        );
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const { result } = await db.is_multi_part_id_users.getManyBy(
          "age",
          mockUser1.age,
        );
