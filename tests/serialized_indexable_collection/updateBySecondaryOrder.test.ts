...
Deno.test.ignore(
  "serialized_indexable_collection - updateBySecondaryOrder",
  async (t) => {
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const { result } = await db.is_users.updateBySecondaryOrder(
            "age",
            updateData,
            {
              limit: 2,
    ...
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const { result: crs } = await db.is_users.updateBySecondaryOrder(
            "age",
            updateData,
            {
              strategy: "replace",
    ...
    await t.step("Should successfully parse and update", async () => {
      await useDb(async (db) => {
        ...
        await db.zis_users.updateBySecondaryOrder("age", mockUser1)
    ...
    await t.step("Should fail to parse and update document", async () => {
      await useDb(async (db) => {
        ...
        await db.zis_users.updateBySecondaryOrder(
          "age",
          mockUserInvalid,
