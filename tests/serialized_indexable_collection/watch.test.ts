...
Deno.test({
  name: "serialized_indexable_collection - watch",
  ...
  fn: async (t) => {
    await t.step("Should receive all document updates", async () => {
      await useDb(async (db) => {
        ...
        await sleep(500)
        await db.is_users.write(id, mockUser2)
        await sleep(500)
    ...
    await t.step("Should not receive unrelated document updates", async () => {
      await useDb(async (db) => {
        ...
        await sleep(500)
        await db.is_users.write(id2, mockUser2)
        await sleep(500)
