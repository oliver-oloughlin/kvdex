...
Deno.test("serialized_indexable_collection - upsert", async (t) => {
  ...
  await t.step("Should upsert document with multi-part id", async () => {
    await useDb(async (db) => {
      ...
      const cr1 = await db.is_multi_part_id_users.upsert({
        id,
        set: mockUser1,
        update: mockUser2,
      });
      ...
      const doc1 = await db.is_multi_part_id_users.find(id);
      assert(doc1 !== null);
      assert(doc1.value.username === mockUser1.username);

      const cr2 = await db.is_multi_part_id_users.upsert({
        id,
        set: mockUser1,
        update: mockUser3,
      }, { strategy: "replace" });
      ...
      const doc2 = await db.is_multi_part_id_users.find(id);
      assert(doc2 !== null);
