...
Deno.test("serialized_indexable_collection - updateOne", async (t) => {
  ...
  await t.step("Should update one document with multi-part id", async () => {
    await useDb(async (db) => {
      ...
      const updateCr = await db.is_multi_part_id_users.updateOne(mockUser3, {
        strategy: "replace",
      });
      assert(updateCr.ok);

      const doc1 = await db.is_multi_part_id_users.find(cr1.id);
      const doc2 = await db.is_multi_part_id_users.find(cr2.id);
