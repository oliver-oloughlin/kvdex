  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const count = await db.is_multi_part_id_users.countBySecondaryOrder(
          "age",
        );
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const startCount = await db.is_users.countBySecondaryOrder("age", {
          startValue: 50,
        });
        assert(startCount === 2);

        // endValue is exclusive
        const endCount = await db.is_users.countBySecondaryOrder("age", {
          endValue: 80,
        });
        assert(endCount === 2);

        // Combined start and end bounds
        const rangeCount = await db.is_users.countBySecondaryOrder("age", {
          startValue: 50,
          endValue: 80,
        });
        assert(rangeCount === 1);
