...
Deno.test("serialized_indexable_collection - forEachByOrder", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        await db.is_users.forEachByOrder(
          "age",
          (doc) => docs.push(doc),
        );
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const docs: string[] = [];
        await db.is_multi_part_id_users.forEachByOrder(
          "age",
          (doc) => {
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        assertEquals(limited, order.slice(0, 2));
      });
    },
  );
});
