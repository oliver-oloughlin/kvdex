...
Deno.test("serialized_indexable_collection - countBy", async (t) => {
  await t.step(
    "Should correctly count total number of documents in the collection by secondary index",
    async () => {
      await useDb(async (db) => {
        const count1 = await db.is_users.countBy(
          "age",
          mockUser1.age,
        );
        assert(count1 === 0);
        ...
        const count2 = await db.is_users.countBy(
          "age",
          mockUser1.age,
        );
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const count = await db.is_multi_part_id_users.countBy(
          "age",
          mockUser1.age,
        );
        assert(count === 2);
