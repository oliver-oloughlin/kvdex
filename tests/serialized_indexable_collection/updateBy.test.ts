...
Deno.test("serialized_indexable_collection - updateBy", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const updateCr = await db.is_users.updateBy(
          "username",
          mockUser1.username,
          updateData,
          {
        ...
        const byPrimary = await db.is_users.findBy(
          "username",
          mockUser1.username,
        );

        const bySecondary = await db.is_users.getManyBy(
          "age",
          mockUser1.age,
        );
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const updateCr = await db.is_multi_part_id_users.updateBy(
          "username",
          mockUser1.username,
          updateData,
          { strategy: "merge-shallow" },
        ...
        const doc = await db.is_multi_part_id_users.findBy(
          "username",
          mockUser1.username,
        );
        assert(doc !== null);
