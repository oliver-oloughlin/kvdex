...
Deno.test("serialized_indexable_collection - updateManyBy", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const { result } = await db.is_users.updateManyBy(
          "age",
          users[0].age,
          updateData,
          {
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const { result: bySecondaryDocs } = await db.is_users
          .getManyBy(
            "age",
            mockUser1.age,
          );
  ...
  await t.step("Should successfully parse and update", async () => {
    await useDb(async (db) => {
      ...
      await db.zis_users.updateManyBy("age", users[0].age, mockUser1)
  ...
  await t.step("Should fail to parse and update document", async () => {
    await useDb(async (db) => {
      ...
      await db.zis_users.updateManyBy(
        "age",
        users[0].age,
        mockUserInvalid,
