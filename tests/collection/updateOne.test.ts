...
Deno.test("collection - updateOne", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const updateCr = await db.users.updateOne(updateData, {
          strategy: "merge-shallow",
        });

        assert(updateCr.ok);

        const doc1 = await db.users.find(cr1.id);
        const doc2 = await db.users.find(cr2.id);
  ...
  await t.step("Should successfully parse and update", async () => {
    await useDb(async (db) => {
      ...
      await db.z_users.updateOne(mockUser1).catch(() => assertion = false);
