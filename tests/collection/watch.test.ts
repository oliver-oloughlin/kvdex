...
Deno.test("collection - watch", async (t) => {
  await t.step("Should receive all document updates", async () => {
    await useDb(async (db) => {
      ...
      const { promise, cancel } = db.users.watch(id, (doc) => {
        docs.push(doc);
      });
  ...
  await t.step("Should not receive unrelated document updates", async () => {
    await useDb(async (db) => {
      ...
      const { promise, cancel } = db.users.watch(id1, (doc) => {
        count++;
        lastDoc = doc;
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const { promise, cancel } = db.multi_part_id_nums.watch(id, (doc) => {
          docs.push(doc as Document<number, [string, number]> | null);
        });

        await db.multi_part_id_nums.set(id, n1);
        await sleep(500);
        await db.multi_part_id_nums.set(id, n2, { overwrite: true });
        await sleep(500);
        await db.multi_part_id_nums.update(id, n3);
        await sleep(500);
        await db.multi_part_id_nums.delete(id);
        await sleep(500);
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const { promise, cancel } = db.multi_part_id_nums.watch(id1, (doc) => {
          count++;
          lastDoc = doc;
          if (doc?.value) {
            n = doc.value;
        ...
        await db.multi_part_id_nums.set(id2, n1);
        await sleep(500);
        await db.multi_part_id_nums.set(id2, n2, { overwrite: true });
        await sleep(500);
        await db.multi_part_id_nums.update(id2, n3);
        await sleep(500);
        await db.multi_part_id_nums.delete(id2);
        await sleep(500);
