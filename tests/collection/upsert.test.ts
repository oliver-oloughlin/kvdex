...
Deno.test("collection - upsert", async (t) => {
  ...
  await t.step("Should upsert document with multi-part id", async () => {
    await useDb(async (db) => {
      ...
      const cr1 = await db.multi_part_id_nums.upsert({
        id,
        set: 10,
        update: 20,
      });
      ...
      const doc1 = await db.multi_part_id_nums.find(id);
      assert(doc1 !== null);
      assertEquals(doc1.value, 10);

      const cr2 = await db.multi_part_id_nums.upsert({
        id,
        set: 10,
        update: 20,
      });
      ...
      const doc2 = await db.multi_part_id_nums.find(id);
      assert(doc2 !== null);
