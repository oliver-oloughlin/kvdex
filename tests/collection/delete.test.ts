  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const deleteCr = await db.users.delete(cr.id);
        assert(deleteCr.ok);
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const deleteCr = await db.users.delete(cr.id, { batched: true });
        assert(deleteCr.ok);
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const deleteCr = await db.multi_part_id_nums.delete(cr.id);
        assert(deleteCr.ok);
