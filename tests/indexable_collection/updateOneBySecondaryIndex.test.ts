...
Deno.test("indexable_collection - updateOneBySecondaryIndex", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const updateCr = await db.i_users.updateOneBySecondaryIndex(
          "age",
          mockUser2.age,
          updateData,
          {
  ...
  await t.step("Should successfully parse and update", async () => {
    await useDb(async (db) => {
      ...
      await db.zi_users.updateOneBySecondaryIndex(
        "age",
        mockUser2.age,
        mockUser1,
