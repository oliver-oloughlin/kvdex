  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        await db.i_multi_part_id_users.set(id3, mockUser3);

        const { promise, cancel } = db.i_multi_part_id_users.watchMany(
          [id1, id2, id3],
          (docs) => {
            snapshots.push(
        ...
        const cr1 = await db.i_multi_part_id_users.set(id1, mockUser1);
        await sleep(500);
        await db.i_multi_part_id_users.delete(id1);
        await sleep(500);
        const cr2 = await db.i_multi_part_id_users.set(id2, mockUser2);
        await sleep(500);
        const cr3 = await db.i_multi_part_id_users.update(id3, {
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        await db.i_multi_part_id_users.set(id4, mockUser1);
        await sleep(500);
        await db.i_multi_part_id_users.set(id4, mockUser2, { overwrite: true });
        await sleep(500);
        await db.i_multi_part_id_users.update(id4, mockUser3);
        await sleep(500);
        await db.i_multi_part_id_users.delete(id4);
        await sleep(500);
