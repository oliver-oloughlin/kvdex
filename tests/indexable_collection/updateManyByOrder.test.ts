...
Deno.test(
  "indexable_collection - updateManyByOrder",
  async (t) => {
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const { result } = await db.i_users.updateManyByOrder(
            "age",
            updateData,
            {
              limit: 2,
          ...
          await db.i_users.forEachByOrder("age", (doc) => {
          ...
          const last = await db.i_users.getOneByOrder("age", {
            reverse: true,
          });
    ...
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const { result: crs } = await db.i_users.updateManyByOrder(
            "age",
            updateData,
            {
              strategy: "replace",
    ...
    await t.step("Should successfully parse and update", async () => {
      await useDb(async (db) => {
        ...
        await db.zi_users.updateManyByOrder("age", mockUser1)
    ...
    await t.step("Should fail to parse and update document", async () => {
      await useDb(async (db) => {
        ...
        await db.zi_users.updateManyByOrder(
          "age",
          mockUserInvalid,
    ...
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const docs = await db.i_users.getManyByOrder("age");
    ...
    await t.step(
      ...
      async () => {
        await useDb(async (db) => {
          ...
          const user2 = await db.i_users.findBy(
            "username",
            mockUser2.username,
          );
          assertEquals(user2?.value.address.country, "Ireland");

          const user1 = await db.i_users.findBy(
            "username",
            mockUser1.username,
          );
