...
Deno.test("indexable_collection - forEachBySecondaryOrder", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        await db.i_users.forEachBySecondaryOrder(
          "age",
          (doc) => docs.push(doc),
        );
  ...
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const docs: string[] = [];
        await db.i_multi_part_id_users.forEachBySecondaryOrder(
          "age",
          (doc) => {
