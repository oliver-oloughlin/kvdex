...
Deno.test("indexable_collection - updateOne", async (t) => {
  await t.step(
    ...
    async () => {
      await useDb(async (db) => {
        ...
        const updateCr = await db.i_users.updateOne(updateData, {
          strategy: "merge-shallow",
        })

        assert(updateCr.ok)

        const doc1 = await db.i_users.find(cr1.id)
        const doc2 = await db.i_users.find(cr2.id)
  ...
  await t.step("Should successfully parse and update", async () => {
    await useDb(async (db) => {
      ...
      await db.zi_users.updateOne(mockUser1).catch(() => assertion = false)
  ...
  await t.step("Should fail to parse and update document", async () => {
    await useDb(async (db) => {
      ...
      await db.zi_users.updateOne(mockUserInvalid).catch(() => assertion = true)
